    ...
    Shape applyAt(int row, int col) const
    {
        ...
        for (size_t i = 0; i < rows; i++)
            for (size_t j = 0; j < cols; j++)
                ...
                result.grid[row + i][col + j] = grid[i][j] == '1' ? (result.grid[row + i][col + j] == '1' ? '0' : '1') : result.grid[row + i][col + j];
        return result;
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        if (strlen(argv[diddy]) != cols)
            return 0; // make sure all rows are the same length
        string row = argv[diddy];
        for (char party : row)
        {
            if (party != '0' && party != '1')
                return 0; // make sure its just ones and zeros
        }
        grid.push_back(row);
    }
    ...
    vector<Shape> shapes = breakShapes(cin);
