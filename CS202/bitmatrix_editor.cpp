...
void usage() {
  ...
  cerr << endl;
  cerr << "EMPTY rows cols Sets the current matrix to a new bit-matrix whose "
          "values are all zeros."
       << endl;
  cerr << "READ fn         Reads a matrix from the file, and sets the current "
          "matrix to be this matrix."
       << endl;
  cerr << "SET row col val Sets the given element of the current matrix to the "
          "given value"
       << endl;
  cerr << "VAL row col     Returns the value of the given element." << endl;
  cerr << "SWAP r1 r2      Swaps the given rows of the current matrix using "
          "Swap_Rows()."
       << endl;
  cerr << "+= r1 r2        Sets r1 of the current matrix equal to r1+r2 using "
          "the R1_Plus_Equals_R2() method."
       << endl;
  cerr << "WRITE fn        Calls the Write() method to write the current "
          "matrix to the specified file."
       << endl;
  cerr << "PRINT [w]       Calls Print(w) -- w is zero if unspecified." << endl;
  cerr << "PGM fn pixels b Calls the PGM() method to create a PGM picture of "
          "the current matrix."
       << endl;
  cerr << "STORE key       Makes a copy of the current matrix and stores it "
          "into the hash table with the key."
       << endl;
  cerr << "RECALL key      This will set the current matrix to a copy of the "
          "bit-matrix stored in the hash table."
       << endl;
  cerr << "ALL key         This calls All() and prints out each key, along "
          "with the dimensions of the bit-matrix stored there."
       << endl;
  cerr << "SUM k1 k2       This adds the two matrices stored with the given "
          "keys and puts the sum into the current matrix."
       << endl;
  cerr << "PRODUCT k1 k2   This multiplies the two matrices stored with the "
          "given keys and puts the product into the current matrix."
       << endl;
  cerr << "SUB r1 r2 ...   This replaces the current matrix with a submatrix "
          "of the specified rows."
       << endl;
  cerr << "INVERT          This inverts the current matrix and replaces it "
          "with the inverse."
       << endl;
  ...
  while (1) {
    ...
    if (sv.size() > 0 && sv[0][0] != '#') {
      if (sv[0] == "EMPTY") {
        ...
        w = 0;
        if (sv.size() > 2 ||
            (sv.size() == 2 && sscanf(sv[1].c_str(), "%d", &w) != 1)) {
