...
// This is basically a for loop creating the 26x26 that will later be refrenced for encrypting and decrypting. It just
// runs through the alphabet.

void VigenereCipher::encrypt(const string inputFile,
                             const string outputFile) const {
string file = fileToString(inputFile);
string keyStream = makeKeystream(file);
string encrypted;

for (size_t a = 0, index = 0; a < file.length(); ++a) {
    if (isalpha(file[a])) {
        char eChar = mVTable[(mKey[index % mKey.length()]) - 'a'][(file[a] - 'a')];
        encrypted += eChar;
        ++index;
    }
    else {
    ...
    stringToFile(encrypted,outputFile);
}

// The encryption happens by first initalizing the variable encrypted which is the text that'll be be turned into gibberish. 
// The for loop happens by looking at each letter, determining if they're in the phonetic alphabet and then changing it in
// accordance to the 26x26. If it isn't a letter, like a dash or punctuation, it stays the same.

void VigenereCipher::decrypt(const string inputFile,
                             const string outputFile) const {
string eWord = fileToString(inputFile);
string keyStream = makeKeystream(eWord);
string decrypted;

for (size_t a = 0, index = 0; a < eWord.length(); ++a) {
        if (isalpha(eWord[a])) {
    ...
    stringToFile(decrypted,outputFile);
                             }

// Decrypt function is almost inline with the encrypt function. Only difference is it takes in a letter and changes it.
...
// Very similar to VigenereCipher object from earlier. This just a print statement in it so the user can see the output.
...
int main(int argc, char **argv) {
 if (argc != 5) {
    ...
    // TA helped me on this, never caught his name.
