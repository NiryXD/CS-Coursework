...
int main(int argc, char *argv[])
{
    ...
    while ((line_len = getline(&line_buffer, &buffer_size, infile)) != -1 && !g_shutdown) {
...
Hello world, this is the world that we live in, so love the world!
But some other worlds might not be like our world.
So behave, and love out worldliness, but not their world!
Yeah, that sounds about right.

~> gcc -std=gnu11 -o tprog main.c -lpthread
~> ./tprog input.txt output.txt world Eurf 10 5

Output file (output.txt)
Hello Eurf, this is the Eurf that we live in, so love the Eurf!
But some other worlds might not be like our Eurf.
So behave, and love out worldliness, but not their Eurf!
Yeah, that sounds about right.

Also no need for error checking, that only adds on to the line count