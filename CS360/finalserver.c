...
// Manual byte swap function for uint16_t (replaces ntohs)
...
int main(int argc, char *argv[])
{
    ...
    while (run){
        ...
        if (cfd >= 0){
            ...
            for (j = 0; j < 100; j+=1){
                if (!clients[j].active){ // should be 0 already
                    clients[j].fd = cfd;
                    clients[j].active = 1;
                    clients[j].time = time(NULL);
                    clients[j].bufferL = 0;
                    clients[j].sentHello = 0;
                    clients[j].recieveHello = 0;
                    
                    memset(clients[j].buffer, 0, sizeof(clients[j].buffer));
                    send(clients[j].fd, "HELLO", 5, 0);
                    clients[j].sentHello = 1;
                    connect = 1;
                    break;
                }
        ...
        for (j = 0; j < 100; j+=1){
            ...
            if ((!clients[j].recieveHello) && (clients[j].sentHello)){
            ...
            // active if client sent the hello
            if (!clients[j].recieveHello){
                if ((bytes >= 5) && (memcmp(temp_buff, "HELLO", 5) == 0)){
                    clients[j].recieveHello = 1;
                } else { 
                    close(clients[j].fd);
                    clients[j].active = 0;
                    clients[j].fd = -1;
                            ...
                            if (status == 0 && results != NULL){
                                ...
                                if (ip > 0){
                                    ...
                                    snprintf(response, sizeof(response), 
                                             ...
                                             swap_bytes(words[0]), swap_bytes(words[1]), 
                                             swap_bytes(words[2]), swap_bytes(words[3]),
                                             swap_bytes(words[4]), swap_bytes(words[5]), 
                                             swap_bytes(words[6]), swap_bytes(words[7]));
                                    send(clients[j].fd, response, strlen(response), 0);
                                }
