...
int RENAME_showUsage(const char *s) {
...
Superball::Superball(int argc, char **argv) {
    ...
    if (argc != 5) {
        exit(RENAME_showUsage(NULL));
    }

    // Convert input arguments to integers and validate them
    try {
        RENAME_rows = stoi(argv[1]); // Convert rows input to integer
        if (RENAME_rows <= 0) exit(RENAME_showUsage("Bad rows"));
    } catch (...) {
        exit(RENAME_showUsage("Bad rows"));
    }

    try {
        RENAME_cols = stoi(argv[2]); // Convert columns input to integer
        if (RENAME_cols <= 0) exit(RENAME_showUsage("Bad cols"));
    } catch (...) {
        exit(RENAME_showUsage("Bad cols"));
    }

    try {
        RENAME_minScoreSize = stoi(argv[3]); // Convert minimum score size input to integer
        if (RENAME_minScoreSize <= 0) exit(RENAME_showUsage("Bad min-score-size"));
    } catch (...) {
        exit(RENAME_showUsage("Bad min-score-size"));
    }
    ...
    for (i = 0; i < static_cast<int>(strlen(argv[4])); i++) {
        if (!isalpha(argv[4][i])) exit(RENAME_showUsage("Colors must be distinct letters"));
        if (!islower(argv[4][i])) exit(RENAME_showUsage("Colors must be lowercase letters"));
        if (RENAME_colorMapping[argv[4][i]] != 0) exit(RENAME_showUsage("Duplicate color"));
    ...
    for (i = 0; i < RENAME_rows; i++) {
        if (!(cin >> RENAME_inputString)) {
            exit(RENAME_showUsage("Bad board: not enough rows on standard input"));
        }
        if (static_cast<int>(RENAME_inputString.size()) != RENAME_cols) {
            exit(RENAME_showUsage("Bad board on row " + to_string(i) + " - wrong number of characters."));
        }
        for (j = 0; j < RENAME_cols; j++) {
            ...
            if (RENAME_inputString[j] != '*' && RENAME_inputString[j] != '.' && RENAME_colorMapping[RENAME_inputString[j]] == 0) {
                exit(RENAME_showUsage("Bad board row " + to_string(i) + " - bad character " + RENAME_inputString[j] + "."));
            }
...
int main(int argc, char **argv) {
    if (argc != 5) return RENAME_showUsage(NULL);
